...
public class BattleState extends GameState {
    ...
    private enum SubState {
        MAIN_MENU, MOVE_SELECTION, BAG, POKEMON, MESSAGE_ONLY, CATCH_ANIMATION
    }
    ...
    public enum Phase {
        PLAYER_TURN, ENEMY_TURN, BATTLE_END
    }
    ...
    private void handleMainMenu() {
        ...
        if (keyHandler.actionPressed) {
            ...
            gamePanel.playSoundEffect("res/sound/select.wav");
            if (selectedOption == 0) {
                changeSubState(SubState.MOVE_SELECTION);
            } else if (selectedOption == 1) {
                changeSubState(SubState.BAG);
    ...
    private void playerAttackAction() {
        ...
        if (playerPokemon.getCurrentHp() <= playerPokemon.getMaxHp() / 3) {
            ...
            if (pType.equals("FIRE") && mType.equals("FIRE")) {
                abilityMod = 1.5;
            } else if (pType.equals("WATER") && mType.equals("WATER")) {
                abilityMod = 1.5;
            } else if (pType.equals("GRASS") && mType.equals("GRASS")) {
                abilityMod = 1.5;
            }
        }
        ...
        } else if (typeMod < 1.0 && typeMod > 0) {
            effectMessage = "It's not very effective...";
        } else if (typeMod == 0.0) {
            effectMessage = "It had no effect...";
        } else {
            effectMessage = "";
        }
    ...
    private void applyItemEffect(Item item) {
        ...
        if (item.getName().toLowerCase().contains("potion")) {
            ...
            if (ballName.contains("great")) {
                catchChance *= 1.5;
            } else if (ballName.contains("ultra")) {
                catchChance *= 2.0;
            }
            if (catchChance < 0.2) {
                catchChance = 0.2;
    ...
    private void handleCatchAnimation() {
        ...
        if (catchPhase == 0) { // MELAYANG
            ...
            if (catchTimer == 1) {
                gamePanel.playSoundEffect("res/sound/poof.wav");
            }
            ...
            if (catchSuccess) {
                ...
                if (success) {
                    battleMessage = "Gotcha! " + enemyPokemon.getName().toUpperCase() + " was caught!";
                } else {
                    battleMessage = "The Ball broke due to a database error!";
                }
                phase = Phase.BATTLE_END;
            } else {
        ...
        if (subState == SubState.CATCH_ANIMATION && catchPhase >= 0 && catchPhase < 4) {
            if (currentBallImg != null) {
                ...
                if (catchPhase == 0) {
                    ...
                    if (catchTimer >= 30 && catchTimer <= 40) {
                        wobbleAngle = -0.3;
                    }
                    if (catchTimer >= 90 && catchTimer <= 100) {
                        wobbleAngle = 0.3;
                    }
                    if (catchTimer >= 150 && catchTimer <= 160) {
                        wobbleAngle = -0.3;
                    }
    ...
    private void renderStatusCard(Graphics2D g2d, int x, int y, Pokemon pokemon, boolean isEnemy) {
        ...
        if (hpPercent > 0.5) {
            g2d.setColor(new Color(65, 225, 65));
